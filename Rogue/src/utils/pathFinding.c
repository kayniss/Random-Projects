...
void addPositionXY(int ** frontier, int frontierCount, int y, int x) {
    frontier[frontierCount][0] = y;
    frontier[frontierCount][1] = x;
}

int checkPathPosition(int y, int x) {
...
int addNeighbours(int ** frontier, int frontierCount, int *** cameFrom, int y, int x) {
    ...
    // north 
    if (y > 0 && cameFrom[y - 1][x][0] < 0 && checkPathPosition(y - 1, x)) {
        addPositionXY(frontier, frontierCount, y - 1, x);
        frontierCount++;
        cameFrom[y - 1][x][0] = y;
        cameFrom[y - 1][x][1] = x;
    }
    ...
    // south
    if (y < (MAX_HEIGHT - 1) && cameFrom[y + 1][x][0] < 0 && checkPathPosition(y + 1, x)) {
        addPositionXY(frontier, frontierCount, y + 1, x);
        frontierCount++;
        cameFrom[y + 1][x][0] = y;
        cameFrom[y + 1][x][1] = x;
    ...
    // east
    if (x < (MAX_WIDTH - 1) && cameFrom[y][x + 1][0] < 0 && checkPathPosition(y, x + 1)) {
        addPositionXY(frontier, frontierCount, y, x + 1);
        frontierCount++;
        cameFrom[y][x + 1][0] = y;
        cameFrom[y][x + 1][1] = x;
    } 
    ...
    // west
    if (x > 0 && cameFrom[y][x - 1][0] < 0 && checkPathPosition(y, x - 1)) {
        addPositionXY(frontier, frontierCount, y, x - 1);
        frontierCount++;
        cameFrom[y][x - 1][0] = y;
        cameFrom[y][x - 1][1] = x;
...
void pathFind(Position * start, Position * end) {
    ...
    addPositionXY(frontier, frontierCount, start->y, start->x);
    frontierCount++;
